...
class TestContinuanceTable(BaseDataTest):
    ...
    def check_continuance_value(self, test_values, column=None):
        if test_values[1] == None:
            if column == None:
                slice_value = self.test_continuance_table.slice(self.session, test_values[0])
            else:
                slice_value = self.test_continuance_table.slice(self.session, test_values[0], column=column)
        else:
            if column == None:
                slice_value = self.test_continuance_table.slice(self.session, test_values[0], test_values[1])
            else:
                slice_value = self.test_continuance_table.slice(self.session, test_values[0], test_values[1], column=column)
        assert_almost_equal(slice_value,
                            test_values[2],
                            2,
                            "Slice with with value is not correct. Actual {0} should be {1}".format(slice_value, test_values[2]))
    ...
    def test_slice_values_preventive(self):
        self.test_continuance_table = ContinuanceTable(name='Test', membership=1000, avg_cost=1000)
        self.test_continuance_table.add_value(self.session, maximum=0, membership=1000, maxed_value=0, preventive_care_value=0)
        self.test_continuance_table.add_value(self.session, maximum=500, membership=500, maxed_value=450, preventive_care_value=50)
        self.test_continuance_table.add_value(self.session, maximum=1000, membership=250, maxed_value=750, preventive_care_value=75)
